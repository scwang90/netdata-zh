...
netdataDashboard.menu = {
    ...
    'ipv4': {
        ...
        info: '系统的IPv4堆栈的度量标准。' +
            ...
            '是因特网协议（IP）的第四个版本。它是互联网上基于标准的网络连接方法的核心协议之一。IPv4是一种无连接协议，用于分组交换网络。它采用了最佳的交付模型，因为它不能保证交付，也不能保证正确的顺序或避免重复交付。这些方面，包括数据完整性，是由上层传输协议解决的，比如传输控制协议（TCP）。<br><br>'
    },

    'ipv6': {
        title: 'IPv6 网络',
        icon: '<i class="fas fa-cloud"></i>',
        info: '系统的IPv6堆栈的度量标准。<a href="https://en.wikipedia.org/wiki/IPv6" target="_blank">Internet Protocol version 6 (IPv6)</a> 是互联网协议（IP）的最新版本，它是一种通信协议，为网络上的计算机提供识别和定位系统，并在互联网上传输流量。IPv6是由互联网工程任务组（IETF）开发的，用于处理人们期待已久的IPv4地址耗尽问题。IPv6的目的是取代IPv4。'
    },
    ...
    'cpu': {
        ...
        info: '系统的每个CPU的详细信息。可以在 <a href="#menu_system">系统概述</a> 板块中查看所有cpu的信息.'
    },
    ...
    'disk': {
        ...
        info: '系统所有磁盘性能信息图表。特别注意以一种与iostat-x兼容的方式提供磁盘性能指标。默认情况下，netdata会防止单个分区和未挂载的虚拟磁盘呈现性能图表。通过在netdata配置文件中配置相关设置，仍然可以启用禁用图表。'
    },
...
netdataDashboard.submenu = {
    ...
    'ipfw.dynamic_rules': {
        title: 'dynamic rules',
        info: '动态规则的数量，由相应的有状态防火墙规则创建。<br><br>'
    },
    ...
    'system.softnet_stat': {
        ...
        info: function (os) {
            ...
            // else
                return '与网络接收相关的cpu软中断统计数据。<br><br>';//'Statistics for CPUs SoftIRQs related to network receive work.';
        }
    },
    ...
    'cpu.softnet_stat': {
        ...
        info: function (os) {
            ...
            return '与网络接收相关的cpu软中断统计数据<br><br>';
        }
    },
...
netdataDashboard.context = {
    'system.cpu': {
        info: function (os) {
            void(os);
            return '总 CPU 使用率 (所有核心 ）. 这里 100 ％ 意味着没有 CPU 空闲时间. 你可以在<a href="#menu_cpu">每个CPU内核</a>查看详细信息 。<br><br>'
    ...
    'system.load': {
        info: '当前系统负载, 也就是说，使用CPU或等待系统资源的进程数量 (通常是CPU和磁盘). 3个指标指的是1、5和15分钟的平均值。系统每5秒计算一次。<br><br>',
        height: 0.7
    },
    ...
    'system.io': {
        info: function (os) {
            void(os);
            var s = '所有物理磁盘的输入和输出. 您可以获得 <a href="#menu_disk">每个磁盘</a> 的详细信息。<br><br>';
            // if (os === 'linux')
    ...
    'system.pgpgio': {
        info: '磁盘的内存分页。这通常是系统的整个磁盘输入输出。<br><br>'
    },
    ...
    'system.entropy': {
        colors: '#CC22AA',
        info: '<a href="https://en.wikipedia.org/wiki/Entropy_(computing)" target="_blank">熵</a>是一组随机数（/ dev / random），主要用于密码学。 如果熵池变空，则需要随机数的进程可能运行得慢得多（这取决于每个程序使用的接口），等待池补充。 理想情况下，具有高熵需求的系统应具有用于该目的的硬件设备（TPM是一种这样的设备）。 您可以安装几个仅限软件的选项，例如hasged，尽管这些选项通常仅在服务器中有用。<br><br>'
    ...
    'system.intr': {
        colors: '#DD5555',
        info: 'CPU中断的总数量。'
        //info: 'CPU中断的总数量。Check <code>system.interrupts</code> that gives more detail about each interrupt and also the <a href="#menu_cpu">CPUs</a> section where interrupts are analyzed per CPU core.'
    },

    'system.interrupts': {
        info: 'CPU中断详情。在 <a href="#menu_cpu">CPUs</a> 板块, 可以查看每个CPU核心的中断情况。<br><br>'
    },

    'system.softirqs': {
        info: 'CPU软中断详情。 在 <a href="#menu_cpu">CPUs</a> 板块, 可以查看每个CPU核心的软中断情况。<br><br>'
    },

    'system.processes': {
        info: '系统进程. <b>Running</b> 展示正在CPU中运行的进程. <b>Blocked</b> 展示那些将要进入CPU执行却不能进入的进程，例如他们正在等待磁盘操作。<br><br>'
    },
    ...
    'system.ctxt': {
        info: '<a href="https://en.wikipedia.org/wiki/Context_switch" target="_blank">Context Switches</a>, 是一个进程、任务或线程到另一个进程的CPU切换。如果有许多进程或线程愿意执行，而可供处理它们的CPU内核非常少，那么系统就会进行更多的上下文切换，以平衡它们之间的CPU资源。整个过程是计算密集型的。上下文切换得越多，系统就越慢。'
    },

    'system.idlejitter': {
        info: '空闲抖动的计算。 产生一个请求睡眠几微秒的线程。 当系统唤醒它时，它会测量已经过了多少微秒。 请求和实际睡眠持续时间之间的差异是空闲抖动。 此数字在实时环境中非常有用，其中CPU抖动会影响服务质量（如VoIP媒体网关）。'
    },

    'system.net': {
        info: function (os) {
            var s = '所有物理网络接口的总带宽。这并不包括lo、vpn、网络桥、IFB设备、键接口等，只有物理网络接口的带宽被聚合。<br><br>';
    ...
    'system.ram': {
        info: '系统物理内存使用情况。<br><br>'
    },

    'system.swap': {
        info: '系统交换内存使用。交换空间是在物理内存（RAM）满时使用的。当系统需要更多的内存资源和RAM是满的时候，内存中的非活动页面被移动到交换空间（通常是磁盘、磁盘分区或文件）。<br><br>'
    },
    ...
    'mem.pgfaults': {
        info: 'A <a href="https://en.wikipedia.org/wiki/Page_fault" target="_blank">页错误</a> 误是一种被称为陷阱的中断，当一个运行的程序访问一个被映射到虚拟地址空间的内存页时，它由计算机硬件产生，但实际上并没有加载到主存中。<br><br>'
    },

    'mem.committed': {
        colors: NETDATA.colors[3],
        info: '已提交的内存，是由进程分配的所有内存的总和。<br><br>'
    },

    'mem.available': {
        info: '可用内存由内核估计，作为用户空间进程可以使用的RAM数量，而不会导致交换。<br><br>'
    },

    'mem.writeback': {
        info: '<b>Dirty</b> 是等待被写入磁盘的内存数量。<b>Writeback</b> 是有多少内存被写进磁盘。'
    },

    'mem.kernel': {
        info: '内核所使用的总内存总量. <b>Slab</b> 是内核用来缓存数据结构以供自己使用的内存数量。<b>KernelStack</b> 是为内核所做的每个任务分配的内存数量。<b>PageTables</b> 是将内存的数量降至最低的页表（一个页表用于将一个虚拟地址转换为物理内存地址）。<b>VmallocUsed</b> 是作为虚拟地址空间使用的内存数量。<br><br>'
    },

    'mem.slab': {
        info: '<b>Reclaimable</b> 是内核可以重用的内存数量。<b>Unreclaimable</b> 表示即使在内存不足的情况下也不能重用。<br><br>'
    },

    'mem.hugepages': {
        info: '专用的（或直接的）HugePages是为配置使用大页面的应用程序保留的内存。Hugepages是使用内存，即使有免费的页面可用。<br><br>'
    },
    ...
    'disk.ops': {
        info: '完成磁盘I/O操作。请记住，请求的操作数量可能更高，因为系统能够合并相邻的操作(参见合并操作图)。'
    },
